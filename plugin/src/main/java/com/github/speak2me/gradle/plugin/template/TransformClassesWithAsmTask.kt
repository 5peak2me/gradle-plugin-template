...
package com.github.speak2me.gradle.plugin.template
...
@CacheableTask
@BuildAnalyzer(
  primaryTaskCategory = TaskCategory.COMPILED_CLASSES,
  secondaryTaskCategories = [TaskCategory.SOURCE_PROCESSING]
)
internal abstract class TransformClassesWithAsmTask : NewIncrementalTask() {
  ...
  class CreationAction(
    ...
    override val name: String = computeTaskName("transform", "ClassesWithZeppx")
    ...
    override fun configure(task: TransformClassesWithAsmTask) {
      ...
      if (creationConfig is ApkCreationConfig) {
        task.profilingTransforms.setDisallowChanges(
          (creationConfig as ApkCreationConfig).advancedProfilingTransforms
        )
        task.shouldPackageProfilerDependencies.setDisallowChanges(
          (creationConfig as ApkCreationConfig).shouldPackageProfilerDependencies
        )
      }
    }
  }
